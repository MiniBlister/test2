 ...
 */
class XiaomiDevice extends ipsmodule
{
    ...
    static $StatusvarProfile = array(
        ...
        "switch" => array("click" => "", "double_click" => "", "voltage" => "~Volt")
    );
...
//Always create our own Splitter, when no parent is already available
        ...
        $this->RegisterTimer('RequestState', 0, 'KoXiDe_RequestState($_IPS[\'TARGET\']);');
        $this->model = "";
    }
...
// Hier kommen nur noch 'report' und 'heartbeat' rein.
    ...
    private function Decode($Ident, $Value)
    {
        ...
        if ((!array_key_exists($Ident, self::$StatusvarProfile[$this->model])) &&
                !array_key_exists($Ident . "_" . trim($Value), self::$StatusvarProfile[$this->model]))
        {
            $this->SendDebug('Decode todo: ' . $Ident, $Value, 0);
            return;
        }
        ...
        switch ($this->model)
        {
            ...
            case 'switch':
                if ($Ident == "status")
                    $this->SetValueBoolean(trim($Value), true);
                if ($Ident == "voltage")
                    $this->SetValueFloat($Ident, intval($Value) / 1000);
                return;
        }
